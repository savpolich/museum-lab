...
@router.get("/custom/types/search", tags=["Аналитика - запросы"])
def search_types_by_name(
    pattern: str = "%изделие%", 
    db: Session = Depends(get_db)
):
    """
    Поиск типов экспонатов по названию.
    - pattern: SQL LIKE-шаблон, по умолчанию '%изделие%'
    - Пример: ?pattern=%картина%
    """
    results = crud.get_types_by_name_pattern(db, pattern=pattern)
    return create_response_with_sql(results)

@router.get("/custom/analytics/items-by-type", tags=["Аналитика - запросы"])
def get_items_count_grouped_by_type(db: Session = Depends(get_db)):
    """Количество экспонатов, сгруппированных по типу"""
    results = crud.get_items_count_by_type(db)
    return create_response_with_sql(results)

@router.get("/custom/analytics/revenue-by-type", tags=["Аналитика - запросы"])
def get_revenue_stats_by_type(db: Session = Depends(get_db)):
    """
    Выручка и средняя прибыльность по типам экспонатов.
    Формула: SUM(price * profit * scale)
    """
    results = crud.get_revenue_by_type(db)
    return create_response_with_sql(results)